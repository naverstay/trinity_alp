...
function builder(el, builderParams) {
    ...
    function parseMarkup(children) {
        ...
        for (let i = 0, li = nodeList.length; i < li; i++) {
            if (nodeList[i] instanceof HTMLElement && nodeList[i].tagName.toUpperCase() === 'TEMPLATE') {
                continue;
            }
    ...
    function insertBefore(node, targetPar) {
        ...
        if (targetPar instanceof HTMLElement && targetPar.tagName.toUpperCase() === 'TEMPLATE') {
            return '';
        } else {
    ...
    function empty() {
        const removed = [];
        let counter = 0;
        while (counter < select.children.length - 1) {
            console.log('select.children', select.children[counter], counter);
